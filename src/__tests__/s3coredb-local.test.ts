...
const authContext: AuthContext = { userPermissions: ["create", "read"], isAdmin: false };
...
describe("S3CoreDB Local Adapter", () => {
  ...
  it("should query nodes with multiple properties", async () => {
    ...
    const query = { type: "user", "properties.name": "Charlie", "properties.age": 25 };
    const results = await db.queryNodes(query);
    expect(results.length).toBe(1);
  ...
  it("should handle relationships between nodes", async () => {
    ...
    const testAuthContext: AuthContext = { userPermissions: ["read", "create", "restricted"], isAdmin: false };
    db.setDefaultAuthContext(testAuthContext);

    // Query: "Who follows User2?" (incoming relationships to User2)
    const followers = await db.queryRelatedNodes(user2.id, "FOLLOWS", testAuthContext, { direction: "IN" });
    ...
    const following = await db.queryRelatedNodes(user1.id, "FOLLOWS", testAuthContext, { direction: "OUT" });
  ...
  it("should respect node permissions", async () => {
    ...
    const createAuthContext: AuthContext = { userPermissions: ["create", "read", "restricted"], isAdmin: false };
    ...
    const limitedAuthContext: AuthContext = { userPermissions: ["read"], isAdmin: false };
    ...
    const fullAuthContext: AuthContext = { userPermissions: ["restricted"], isAdmin: false };
